(function() {
	...
	function findCardForOpenSheet() {
		...
		const all = Array.from(document.querySelectorAll('.gram__card'));
		return all.find(c => {
			const t = (c.dataset.title || c.querySelector('.gram__card__title')?.textContent || '').trim();
			return t === title;
		}) || null;
	}
	...
	function applyModelSelectionToSheet(card) {
		if (!card) return;
		const basePriceStr = card.dataset.price || (card.querySelector('.gram__price')?.textContent) || '';
...
function applyFromHash() {
	...
	$$(".gram__chip[data-category]").forEach(c => {
...
(() => {
	...
	$$(".gram__nav__menu [data-filter-category]").forEach(link => {
...
const SEARCH = (() => {
	...
	function ensureOriginal(card) {
		if (!original.has(card)) {
			const t = card.querySelector(".gram__card__title");
			const d = card.querySelector(".gram__card__desc");
			original.set(card, {
				title: t ? t.innerHTML : "",
				desc: d ? d.innerHTML : ""
			});
	...
	function buildIndex() {
		...
		cards.forEach(card => {
			ensureOriginal(card);
			const title = card.dataset.title || card.querySelector(".gram__card__title")?.textContent || "";
			const desc = card.dataset.desc || card.querySelector(".gram__card__desc")?.textContent || "";
	...
	function highlight(card, terms) {
		...
		const tEl = card.querySelector(".gram__card__title");
		const dEl = card.querySelector(".gram__card__desc");
...
function updateSubcategoriesVisibility(category) {
	const subBar = document.querySelector(".gram__bar__subcategories");
	if (!subBar) return;
	subBar.style.display = "";
	const labels = subBar.querySelectorAll("label.gram__chip");
...
$$(".gram__chip[data-category]").forEach(chip => {
	chip.addEventListener("click", () => {
		const cat = chip.getAttribute("data-category");
		$$(".gram__chip[data-category]").forEach(c => c.dataset.active = "false");
...
(function bootFromQueryWhenNoHash() {
	...
	function availableChipCats() {
		return $$(".gram__chip[data-category]").map(ch => norm(ch.dataset.category));
	}
	...
	function applyMultiCategoryFilter(cats, sub) {
		...
		if (cats.length === 1) {
			const cat = cats[0];
			$$(".gram__chip[data-category]").forEach(c => c.dataset.active = (norm(c.dataset.category) === cat) ? "true" : "false");
...
(() => {
	...
	async function toggleFullscreen() {
		const host = sliderEl.closest(".gram__sheet__media") || sliderEl;
