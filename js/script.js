...
function mountImages(imgs) {
	...
	if (images.length === 0) {
		sliderTrack.innerHTML = ' < div class = "gram__slider__item" > < div style = "display:grid;place-items:center;height:100%;color:var(--muted);padding:24px" > No preview available < /div> < /div>';
		sliderThumbs.innerHTML = '';
	}
	setSlide(0, false);
}
...
Array.from(document.querySelectorAll('.gram__card')).forEach(card => {
	card.addEventListener('click', e => {
		const isButton = e.target.closest('a.gram__btn'); // allow primary link navigation
		if (isButton) return;
