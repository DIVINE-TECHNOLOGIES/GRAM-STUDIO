// ===== A11y dropdowns (unchanged logic, selectors updated) =====
(function() {
	...
	document.querySelectorAll('.auricartisan__nav__menu [data-filter-category]').forEach(link => {
		link.addEventListener('click', e => {
			...
			document.querySelectorAll('.auricartisan__chip[data-category]').forEach(ch => ch.dataset.active = (ch.dataset.category === c) ? "true" : "false");
		});
	});
})();
// Stubs from catalog (safe if grid absent)
...
document.querySelectorAll('.auricartisan__chip[data-category]').forEach(chip => {
	chip.addEventListener('click', () => {
		const cat = chip.getAttribute('data-category');
		document.querySelectorAll('.auricartisan__chip[data-category]').forEach(c => c.dataset.active = "false");
...
// ===== FAQ logic: search, filter, highlight, deep-link =====
(function() {
	...
	const chips = Array.from(document.querySelectorAll('.auricartisan__faq__groups .auricartisan__chip'));
	...
	function setCount() {
		...
		if (count) count.textContent = `${visible} answer${visible===1?'':'s'} • zero fluff`;
	}
	...
	function highlight(el, term) {
		...
		for (const node of Array.from(el.childNodes)) {
			if (node.nodeType === 3) {
				...
				const parts = node.textContent.split(new RegExp(re.source, 'i'));
				if (parts.length > 1) {
					parts.forEach(part => {
						if (re.test(part)) {
	...
	function apply() {
		...
		items.forEach(d => {
			const groups = (d.getAttribute('data-groups') || '').split(',').map(s => s.trim()).filter(Boolean);
			const groupPass = (activeGroup === 'all' || groups.includes(activeGroup));
			const summaryText = d.querySelector('summary')?.textContent || '';
			const answerText = d.querySelector('.auricartisan__faq__answer')?.textContent || '';
			const t = normalize(summaryText + ' ' + answerText);
			const textPass = !term || t.includes(term);
			d.style.display = (groupPass && textPass) ? '' : 'none';
			if (groupPass && term && textPass) {
				d.open = true;
				highlight(d, q);
			} else {
				highlight(d, '');
		...
		setCount();
		try {
			sessionStorage.setItem('faq.group', activeGroup);
			sessionStorage.setItem('faq.q', q);
		} catch (_) {}
	}
	if (search) {
	...
	chips.forEach(ch => {
		ch.addEventListener('click', () => {
			...
			activeGroup = ch.getAttribute('data-group') || 'all';
			apply();
		});
	});
	...
	try {
		const g = sessionStorage.getItem('faq.group');
		const s = sessionStorage.getItem('faq.q');
		if (g && document.querySelector(`.auricartisan__faq__groups .auricartisan__chip[data-group="${g}"]`)) {
			activeGroup = g;
			chips.forEach(c => c.dataset.active = (c.getAttribute('data-group') === g) ? 'true' : 'false');
		}
		if (s) {
			q = s;
